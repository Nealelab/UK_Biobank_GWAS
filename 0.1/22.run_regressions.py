...
if contig not in set(['autosomes', 'chrX', 'chrXY']):
    raise ValueError('Invalid contig "{}" - must be one of {"autosomes", "chrX", "chrXY"}.'.format(contig))
...
vds_variants = hc.read('gs://ukb31063-mega-gwas/qc/ukb31063.gwas_variants.{}.vds'.format(contig))
kt_covariates = hc.read_table('gs://ukb31063-mega-gwas/qc/ukb31063.gwas_covariates.{}.kt'.format(sex))

if pipeline_type == 'phesant':
    kt_phenotype = hc.read_table('gs://ukb31063-mega-gwas/phenotype-pipelines/final-phesant/ukb31063.{1}.{0}.pipeline.{2}.kt'.format(pipeline_type, sex, pipeline_number))
else:
    kt_phenotype = hc.read_table('gs://ukb31063-mega-gwas/phenotype-pipelines/{0}/ukb31063.{1}.{0}.pipeline.{2}.kt'.format(pipeline_type, sex, pipeline_number))
...
vds = (hc.import_bgen('gs://fc-7d5088b4-7673-45b5-95c2-17ae00a04183/imputed/ukb_imp_chr{}_v3.bgen'.format(import_expr),
                      ...
                      tolerance=0.2)
         .annotate_variants_vds(vds_variants, expr='va.keep = true')
         .filter_variants_expr('isDefined(va.keep)', keep=True)
...
if pipeline_type == 'curated':
    path = 'gs://ukb31063-mega-gwas/results-tables/curated/ukb31063.{0}.curated.{1}.kt'.format(sex, contig)
else:
    path = 'gs://ukb31063-mega-gwas/results-tables/{0}/ukb31063.{1}.{0}.{2}.pipeline.{3}.kt'.format(pipeline_type, sex, contig, pipeline_number)
