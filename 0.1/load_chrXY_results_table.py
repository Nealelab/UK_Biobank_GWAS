...
for i in range(pipelines[sex]):
    ht = hl.read_table('gs://ukb31063-mega-gwas/phenotype-pipelines/final-phesant/ukb31063.{0}.phesant.pipeline.{1}.kt'.format(sex, i))
    fields = set([x for x in ht.row])
    if '20003_1141188594' in fields:
        print(i)
...
for phenotype in phenotypes[sex]:
    ...
    ht = hl.import_table('gs://ukb31063-mega-gwas/chrXY-results-tsvs/{0}.imputed_v3.results.{1}.tsv.gz'.format(phenotype, sex),
...
ht_union.write('gs://ukb31063-mega-gwas/results-0.2-tables/gwas.imputed_v3.{}.chrXY.ht'.format(sex), overwrite=True)
